...
class CameraView(object):
    def __init__(
        ...
        # Pre-init in E/S/D order so set_color's positional labels stay correct.
        self.totalCount = {"base": 0, "green_1_stalk": 0, "double_stalk": 0}
        # Manual counting (toggle build): Enter flags a count, consumed in updateTracks.
        self.manual_count_requested = False
        self._manual_count_lock = threading.Lock()
    ...
    def updateTracks(self, detectionsByColor, frameNumber):
        ...
        for configName, detections in detectionsByColor.items():
            ...
            for track, (centroid, bbox) in matchedPairs:
                ...
                # Line-crossing auto-count removed in the toggle build.
                # Counting now happens manually on ENTER (see manual-count block below).
        ...
        # Manual counting (toggle build): if ENTER was pressed, count the current
        # dominant detection once. Priority: double_stalk > green_1_stalk > base.
        with self._manual_count_lock:
            do_count = self.manual_count_requested
            self.manual_count_requested = False
        if do_count:
            self._countCurrentDetection(detectionsByColor)

    def _countCurrentDetection(self, detectionsByColor):
        """Count the current dominant detection (called when ENTER is pressed)."""
        for configName in ("double_stalk", "green_1_stalk", "base"):
            if len(detectionsByColor.get(configName, [])) > 0:
                if configName not in self.totalCount:
                    self.totalCount[configName] = 0
                self.totalCount[configName] += 1
                self.publisher.set_color(configName, self.totalCount)
                print(f"[MANUAL COUNT] {configName} -> {self.totalCount[configName]}  "
                      f"(E:{self.totalCount.get('base',0)} "
                      f"S:{self.totalCount.get('green_1_stalk',0)} "
                      f"D:{self.totalCount.get('double_stalk',0)})")
                return
        print("[MANUAL COUNT] ENTER pressed but nothing detected right now — not counted.")
...
class VisionApplication(object):
    ...
    def _enterCountListener(self):
        """Block on stdin; each ENTER press flags the positioning camera to count."""
        while self.running:
            try:
                input()   # blocks until ENTER (empty prompt)
            except (EOFError, KeyboardInterrupt):
                break
            except Exception:
                break
            cam = self.cameraList.get("PositioningCamera")
            if cam is not None:
                with cam._manual_count_lock:
                    cam.manual_count_requested = True

    def cameraThread(self):

        # Manual counting (toggle build): press ENTER in this terminal to count the
        # current detection instead of auto-counting when a stalk crosses the line.
        threading.Thread(target=self._enterCountListener, daemon=True).start()
        print("Manual counting ENABLED: press ENTER in this terminal to count the current detection.")
