...
class NavStateMachine(Node):
    ...
    def __init__(self):
        ...
        # Allow runtime parameter tuning via 'ros2 param set'
    ...
    def _on_param_change(self, params):
        ...
        param_map = {
            'wall_distance_mm': 'wall_setpoint',
            'front_stop_distance_mm': 'front_stop_dist',
            'collision_distance_mm': 'collision_dist',
            'drive_duty_fraction': 'drive_duty_frac',
            'turn_duty_fraction': 'turn_duty_frac',
            'kp': 'kp',
            'ki': 'ki',
            'kd': 'kd',
            'turn_clear_distance_mm': 'turn_clear_dist',
        }
        for param in params:
            if param.name in param_map:
                setattr(self, param_map[param.name], param.value)
